      ...
      <div className="card p-8 sm:p-10 mb-8">
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 border-t border-borderBrand pt-8 sm:grid-cols-3">
          ...
          <div>
            ...
            <a href={siteConfig.phoneHref} className="block text-sm text-slate-600 hover:text-action transition-colors">
        ...
        <ul className="grid gap-3 sm:grid-cols-2">
          {coreCompetencies.map((item) => (
            <li key={item} className="flex items-start gap-3 text-sm text-slate-700">
              <span className="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-action" />
              {item}
            </li>
          ))}
        </ul>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-borderBrand bg-soft">
                <th className="px-5 py-3 text-left grid-label font-normal">Code</th>
                <th className="px-5 py-3 text-left grid-label font-normal">Description</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-borderBrand">
              {naicsCodes.map((n) => (
                <tr key={n.code}>
                  <td className="px-5 py-3 font-mono font-semibold text-navy">{n.code}</td>
                  <td className="px-5 py-3 text-slate-600">{n.desc}</td>
                </tr>
              ))}
            </tbody>
          </table>
