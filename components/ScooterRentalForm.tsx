...
import { addOnItems } from '@/lib/scooter-rental';
  ...
  return (
    <form className="card p-6 sm:p-8" onSubmit={handleSubmit} noValidate>
      ...
      <label className="mt-6 grid gap-2 text-sm font-medium text-navy">
        ...
        <select name="rentalPlan" required defaultValue="" className="link-ring rounded-2xl border border-borderBrand bg-white px-4 py-3 text-sm text-ink">
          <option value="" disabled>Select one</option>
          <option value="weekly">Weekly — $155/week</option>
          <option value="monthly">Monthly — $550/month</option>
        </select>
      </label>

      <fieldset className="mt-6">
        <legend className="text-sm font-semibold text-navy">Optional add-ons</legend>
      ...
      <div className="mt-6 grid gap-3 text-sm text-slate-700">
        ...
        <label className="flex items-start gap-3"><input type="checkbox" name="consentTerms" value="true" required className="mt-1" /><span>I have read the rental terms and agree to the $305 onboarding charge ($155 first week + $150 refundable deposit) if approved.</span></label>
      </div>
      ...
      <div className="mt-8 flex flex-col gap-4 border-t border-borderBrand pt-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button type="submit" disabled={submitting} className="link-ring inline-flex items-center justify-center rounded-full bg-action px-6 py-3 text-sm font-semibold text-white transition hover:bg-navy disabled:cursor-not-allowed disabled:opacity-70">{submitting ? 'Submitting...' : 'Request Scooter Rental'}</button>
      </div>
    </form>
  );
}
