  ...
  return (
    <footer className="border-t border-borderBrand bg-white">
      <div className="container-shell grid gap-12 py-14 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
        <div>
          <div className="text-lg font-semibold uppercase tracking-[0.18em] text-navy">Already Here LLC</div>
          <p className="mt-4 max-w-xl text-sm leading-7 text-slate-600">
            Onsite infrastructure execution and technical field operations for MSPs, commercial sites, retail environments, and critical systems across Arizona.
          </p>
          <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">
            ...
            <span className="rounded-full border border-borderBrand px-3 py-1">Commercial & Multi-Site Support</span>
        ...
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500">Arizona field markets</h2>
        ...
        <div>
          ...
          <address className="mt-4 not-italic space-y-2 text-sm text-slate-600">
            ...
            <p><a href={siteConfig.phoneHref} className="hover:text-action transition-colors">{siteConfig.phoneDisplay}</a><span className="ml-2 text-xs text-slate-400">— {siteConfig.phoneNote}</span></p>
            <p><a href={`mailto:${siteConfig.email}`} className="hover:text-action transition-colors">{siteConfig.email}</a></p>
          </address>
          <Link href="/dispatch" className="link-ring mt-6 inline-flex items-center justify-center rounded-full bg-action px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-navy">Request Dispatch</Link>
        </div>
      </div>
