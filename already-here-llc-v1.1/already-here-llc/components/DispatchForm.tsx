    ...
    <form className="card p-6 sm:p-8" onSubmit={handleSubmit} encType="multipart/form-data" noValidate>
      ...
      <label className="mt-6 grid gap-2 text-sm font-medium text-navy">
        ...
        <textarea
          ...
          placeholder="Scope summary, arrival instructions, site conditions, hardware details, and anything that affects dispatch planning."
        />
      </label>
      ...
      <label className="mt-6 grid gap-2 text-sm font-medium text-navy">
        ...
        <input
          ...
          type="file"
          accept={acceptedList}
          className="link-ring rounded-2xl border border-dashed border-borderBrand bg-slate-50 px-4 py-3 text-sm text-slate-600 file:mr-4 file:rounded-full file:border-0 file:bg-navy file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white"
        />
        <span className="text-xs font-normal text-slate-500">Accepted: PDF, JPG, JPEG, PNG. Max 10 MB.</span>
      </label>

      {state.error ? (
        <div className="mt-6 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{state.error}</div>
      ) : null}
      ...
      <div className="mt-8 flex flex-col gap-4 border-t border-borderBrand pt-6 sm:flex-row sm:items-center sm:justify-between">
        <p className="max-w-2xl text-sm leading-6 text-slate-500">
          Use this form to submit scope, scheduling requirements, and supporting files. The dispatch endpoint must be configured before the form can deliver production submissions.
        </p>
        <button
          type="submit"
