  ...
  return (
    <>
      ...
      <section className="container-shell py-16 lg:py-24">
        ...
        <p className="section-copy">Operational, technical, trustworthy, field-ready, and vendor capable. No generic consumer repair language.</p>
        <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {buyerReasons.map((reason) => <div key={reason} className="card p-6"><p className="text-sm leading-7 text-slate-700">{reason}</p></div>)}
        </div>
      </section>
        ...
        <div className="container-shell py-16 lg:py-24">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
            <div>
              ...
              <p className="section-copy">Onsite remediation, smart hands support, network troubleshooting, rollout recovery, infrastructure assessment, and closeout-heavy execution.</p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              {['Onsite remediation for issues remote teams cannot close remotely', 'Network, Wi-Fi, rack, endpoint, and commercial infrastructure support', 'Retail, commercial, MSP, vendor, and critical-systems field execution', 'Structured closeout with field notes, photos when permitted, and escalation detail'].map((item) => <div key={item} className="rounded-3xl border border-borderBrand bg-soft px-6 py-6 text-sm leading-7 text-slate-700">{item}</div>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-16 lg:py-24">
        <div className="container-shell grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <div className="mt-8 grid gap-4">{audience.map((item) => <div key={item.title} className="card p-6"><h3 className="text-xl font-semibold text-navy">{item.title}</h3><p className="mt-3 text-sm leading-7 text-slate-600">{item.description}</p></div>)}</div>
          </div>
          <div className="card p-8 sm:p-10">
            <h2 className="text-2xl font-semibold text-navy">Documented work types</h2>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">{documentedWorkTypes.slice(0, 14).map((item) => <li key={item} className="rounded-2xl border border-borderBrand bg-soft px-4 py-3 text-sm text-slate-700">{item}</li>)}</ul>
            <div className="mt-8"><h3 className="text-lg font-semibold text-navy">Operating environments</h3><div className="mt-4 flex flex-wrap gap-2">{environments.map((environment) => <span key={environment} className="rounded-full border border-borderBrand px-3 py-2 text-sm text-slate-700">{environment}</span>)}</div></div>
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-borderBrand bg-soft py-16 lg:py-24">
        <div className="container-shell grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
          <div><span className="eyebrow">Service area</span><h2 className="section-title mt-5">Phoenix-based with project coverage available nationwide depending on client scope.</h2><p className="section-copy">Phoenix-based, with nationwide project coverage available for qualified client scope, scheduling, and travel requirements.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/dispatch" className="link-ring inline-flex items-center justify-center rounded-full bg-action px-6 py-3 text-sm font-semibold text-white transition hover:bg-navy">Request Dispatch</Link><Link href="/who-we-serve" className="link-ring inline-flex items-center justify-center rounded-full border border-borderBrand px-6 py-3 text-sm font-semibold text-navy transition hover:border-action hover:text-action">Who We Serve</Link></div></div>
          <div className="card p-8 sm:p-10"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{markets.map((market) => <div key={market} className="rounded-2xl border border-borderBrand bg-white px-4 py-3 text-sm font-medium text-slate-700">{market}</div>)}</div></div>
        </div>
      </section>
